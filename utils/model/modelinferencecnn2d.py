...
def load_tiles( folder,mean,std,startrow,startcol,endrow,endcol):
        ...
        files = [os.path.join(folder, file) for file in os.listdir(folder) if file.endswith("tif") and file[0] in ['4','7']]
        ...
        imgmedian=img[...,1:13]
        imgmax=img[...,23:]
        img=np.concatenate([imgmedian,imgmax],axis=-1)
...
def predresult(batchsize,patchsize,startrow,startcol,endrow,endcol,predictarraydir,predictjpgdir):
...
if __name__ == "__main__":
    ...
    batchsize=256*5
    inputtag='12median3max'
    mean=np.load(r"D:\ricemodify\limiteddataset\2022pathsize11\sample459xrf4103_mean15.npy")
    std=np.load(r"D:\ricemodify\limiteddataset\2022pathsize11\sample459xrf4103_std15.npy")
    year,patchsize= 2022,11#modeldir.split('\\')[-1]
    modeldir=r"D:\ricemodify\runRFDL\train\202212median3max_dualseparablecnn2d\log"
    ...
    for area in arealist:
        ...
        if area=='linshui':
            ...
            img_list = [file for file in os.listdir(img_dir) if file.endswith('.tif')]
            orgintif=os.path.join(img_dir,img_list[0])
            startrow,startcol,endrow,endcol=4000,4500,5000,5500
        if area=='quxian':
            img_dir=r"D:\ricemodify\datasetRF\origin\quxian2022"
            img_list = [file for file in os.listdir(img_dir) if file.endswith('.tif')]
            orgintif=os.path.join(img_dir,img_list[0])
            startrow,startcol,endrow,endcol=4000,1000,6000,3000#3000,0, 5000,1000
        ...
        for modeltag in os.listdir(modeldir): 
            ...
            if modeltag[0:2] in ["-2"]:
                saveVersion=modeltag#'dualseparablecnn2d'
                print(saveVersion)
                modelpath=os.path.join(modeldir,modeltag)
                modelpath=[os.path.join(modelpath, f) for f in os.listdir(modelpath) if f.startswith('1') and f.endswith('.h5')][0] # [3 1 2] 3是返回                #with open(logpath, 'r') as log_file:
            
                    
                img,trans=load_tiles(img_dir,mean,std,startrow,startcol,endrow,endcol)
                predictdir=rf'D:\ricemodify\runRFDL\predict\{area}\{year}{inputtag}dualseparablecnn2d'
            ...
            # savepredarraypath,savepredjpgpath,savetifpath=os.path.join(predictarray,saveVersion),os.path.join(predictjpg,saveVersion),os.path.join(predicttif,saveVersion)
                labelarray= predresult(batchsize,patchsize,startrow,startcol,endrow,endcol,predictarraydir,predictjpgdir)
