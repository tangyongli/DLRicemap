...
def dataprogress(savetrainxPath, savetrainyPath, savevalxPath, savevalyPath,median12max3,median12max4,median13max3,median13max4):
    ...
    # xtrain, X_temp, ytrain, y_temp = train_test_split(xtrain, ytrain, test_size=0.95, random_state=42)
    ...
    xtrain, X_temp, ytrain, y_temp = train_test_split(x,y, test_size=0.5, random_state=42)
...
def train( modeltag,numfilters,sattention111,sattention011 ,multscalesattetion,csattention,multscalesattetion001,noattention,concatdense,concatcnntrue1d,dropout,l2,agu,epochs,weight=True,learning_rate=0.0001):
        ...
        if modeltag=='dualseparablecnn2d':
            ...
            saveVersion=f't{traincounts}val{valcounts}_depthwise{sattention111}xsat011{sattention011}xmscalesatt{multscalesattetion}xms001{multscalesattetion001}xnoatt{noattention}_catcnn1dtrue{concatcnntrue1d}cbrm0_drop{dropout}_l2{l2}'
            print(saveVersion)
            print('222222222') 
            if weight:
