...
def ResBlock(inputs,channel): # 11 7 7 11
    x = layers.Conv2D(channel, 3, strides=1,padding="same",kernel_initializer='he_normal', use_bias=False)(inputs)
    x = BatchNormalization()(x) # 33 33 64
    x=layers.ReLU()(x)
    # x = layers.Conv2D(channel, 3,  strides=1,padding="same")(x) # 33 3364
    # x = BatchNormalization()(x)
    # x=layers.ReLU()(x)
    # if inputs.shape[3]==x.shape[3]:
    #     x = layers.Add()([inputs, x])  # padding='same'后似乎没有填充？ValueError: Inputs have incompatible shapes. Received shapes (33, 33, 64) and (9, 9, 64)
    # else:
    inputs=layers.Conv2D(channel, (1,1), padding="same",kernel_initializer='he_normal', use_bias=False)(inputs)
...
def dualsparableCnn2d(inputtag,inputshape,numfilters,sattention111,sattention011, multscalesattetion,multscalesattetion001,csattention,noattention,concatdense,concatcnntrue1d,dropout=0,L2=0):
    if inputtag==0:
