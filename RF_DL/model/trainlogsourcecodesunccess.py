...
def dataprogress(savetrainxPath, savetrainyPath, savevalxPath, savevalyPath):
    ...
    meanpath=os.path.join(os.path.dirname(savetrainxPath),f'mean{mean.shape[0]}_{ savemeanstdtag}.npy')
    stdpath=os.path.join(os.path.dirname(savetrainxPath),f'std{std.shape[0]}_ {savemeanstdtag}.npy')
    np.save(meanpath,mean)
    np.save(stdpath,std)
    return xtrain,xval,ytrain,yval,meanpath,stdpath
...
if __name__ == "__main__":
    ...
    savetrainxPath=r"D:\ricemodify\datasetRF\pathsize11\2022\medianq75max\trainxgrid61324nomeanstd_10708x11x11x39.npy"#"D:\ricemodify\datasetRF\pathsize11\traingrid61324\new2022trainxgrid61324nomeanstd_10708x11x11x9.npy"
    savetrainyPath=  r"D:\ricemodify\datasetRF\pathsize11\2022\medianq75max\trainygrid61324_10708x11x11x5.npy"#"D:\ricemodify\datasetRF\pathsize11\traingrid61324\new2022trainygrid61324_10708x11x11x2.npy"
    savevalxPath= r"D:\ricemodify\datasetRF\pathsize11\2022\medianq75max\valxgrid1729nomeanstd_7031x11x11x39.npy"#"D:\ricemodify\datasetRF\pathsize11\valgrid1729\new2023valxgrid1729nomeanstd_7031x11x11x9.npy"
    savevalyPath= r"D:\ricemodify\datasetRF\pathsize11\2022\medianq75max\valygrid1729_7031x11x11x5.npy"#"D:\ricemodify\datasetRF\pathsize11\valgrid1729\new2023valygrid1729_7031x11x11x2.npy"
    ...
    if inputtag=='max':
          inputtype=3
    agu=True
    xtrain,xval,ytrain,yval,meanpath,stdpath= dataprogress(savetrainxPath, savetrainyPath, savevalxPath, savevalyPath)
    ...
    start=time.time()
    def train( depthwisecbrm,depthwisecorrd,ecabeforedense,cbrmbeforedense,concat,dropout,l2,agu):
        model=medianCnn2d(inputshape=(h,w,c),channelratio=8,inputtag=inputtype,depthwisecbrm=depthwisecbrm,depthwisecorrd=depthwisecorrd,ecabeforedense=ecabeforedense,cbrmbeforedense=cbrmbeforedense,concat=concat,dropout=dropout,L2=l2)#inputshape,channelratio,bandswithindex=False,bandposition=14,indexposition=17,sar=False,dropout=0.2,L2=0
       
        model.compile(optimizer=optimizer ,loss= loss, metrics=['accuracy'])
        start=time.time()
         # log
        saveVersion=f'{inputtag}(13)_agu{agu}_depthwisecbrm{depthwisecbrm}_depthwisecorrd{depthwisecorrd}__ecabfdense{ecabeforedense}_cbrmbfdense{cbrmbeforedense}__cat{concat}_drop{dropout}_l2{l2}' #centerpixelcatcenter
        workEnv= 'D:/ricemodify/runRFDL/train'
        checkpointDir = os.path.join(workEnv, f'patchsize{patchsize}/{year}/{saveVersion}') #/train/model is wrong
        ...
        resumeModelpath=os.path.join(resumeModelDir, f'{savemeanstdtag}.h5')
        saveModelPath = os.path.join(checkpointDir,  f'{savemeanstdtag}.h5')
        print('saveModelPath',saveModelPath)
        plotPath = os.path.join(checkpointDir, f'{savemeanstdtag}_lossaccuracy.png')
        confusion_matrixpath=os.path.join(checkpointDir, f'{savemeanstdtag}_confusion_matrix.png')
        callback_ = [
                    ...
                    callbacks.CSVLogger(filename=os.path.join( checkpointDir ,  savemeanstdtag+ '.csv')),
        ...
        total=(end-start)/60
        # Configure logging
        log_file_path=os.path.join(checkpointDir, savemeanstdtag+'.txt')
        logging.basicConfig(
        level=logging.INFO,
    ...
    depthwisecbrm=True
    ecabeforedense=True
    cbrmbeforedense=False
    depthwisecorrd=False
    concat=False
    for agu in [False,True]:
